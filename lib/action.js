...
async function getProjectPath(projectName) {
  ...
  if (settings.projects.length === 0) {
    term.red('>>> ').styleReset('No projects to get path :(');
    term.yellow('>>> ').styleReset(
      `cd /till/project/directory/ and run ${term.str.yellow('pm add')} to add projects and get started`,
    );
    return;
  }
  ...
  if (!projectName) {
    ...
    if (!selectedProject) {
      term.red('>>>').styleReset(
        `Project does not exist. Add it using ${term.str.yellow('pm add [projectPath]')} 
        or cd till the project folder and type ${term.str.yellow('pm add')}`,
      );
      return;
    }
  } else {
